...
### Features
- Add `aes` command rendering an AES (Agentic Engineering System) workflow assessment report — a team-reported self-assessment of one delivery workflow's governance, shared-knowledge and customer-value stocks, its define/deliver/detect activities and director/performer/assessor modes. Every report now leads with a **Decision and next experiment** section (what to delegate now, what is deferred, top actions, and the evidence that would justify reconsidering), and each stock rating shows whether it was actually verified or only assumed — an unverified rating downgrades the reported posture (e.g. to "Possible stretched agent-native — confirmation needed") instead of stating it more confidently than the evidence supports. Supports `--json`, `--html <path>`, and `--file <path>` to render a real assessment; defaults to a fictional FableCart fixture until a real assessment-authoring path ships. This is a separate lens from Fluency Score (an individual's tool usage) and Dark Factory Readiness (a repository's observable controls) — see `docs/features/AES-WORKFLOW-ASSESSMENT.md`

### Bug Fixes
- **AES posture is now only driven by the two foundation stocks (governance, shared knowledge), not customer value.** Previously, a weak or unknown `customerValue` rating alone could flip the reported posture (e.g. to `stretched-agent-native` or `unclear`) even when governance and shared knowledge were solid. `customerValue` is the framework's separately-reported outcome stock and should never gate the governance/shared-knowledge posture axis.
- `aes --file <path>` now validates the JSON against the `AesWorkflowAssessment` shape (required fields, enum values, and the current `schemaVersion`) instead of blindly casting it, so a malformed or out-of-date file produces a clear error instead of a crash or a silently wrong report
- The `--html` export now renders activity notes (previously only shown by the VS Code webview section) and derives every CSS class from a fixed allowlist instead of interpolating rating/state values from the assessment file directly, closing a possible attribute-injection path for a maliciously crafted `--file`
- Reworded the `healthy-agent-native` / `healthy-but-underused` posture guidance, which said governance and shared knowledge must be "strong" even though a `developing` rating is also treated as solid
