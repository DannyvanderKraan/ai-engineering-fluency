...
### Features
- The Repository PRs and Cloud Agent tabs now cache GitHub activity **per entity** instead of only as an aggregate snapshot: each pull request and each cloud-agent task is remembered with its exact `updated_at`, and a revalidation reuses everything that has not changed. In practice a Cloud Agent refresh now spends its task-detail budget on new, changed and previously failed tasks rather than re-fetching up to 200 unchanged ones. The authoritative listing still runs every refresh, so new, changed, removed, archived and moved items are always discovered. Cache files are scoped to the VS Code mode, the GitHub host and a non-reversible hash of the signed-in account (never the login itself), so signing out, switching account or switching Enterprise host can never serve the previous identity's data; Clear Cache removes every scope's files without signing you out, and signing out purges that account's. Both freshness banners now say when the snapshot is revalidating and when the figures are a lower bound (an incomplete listing, or an exhausted task-detail budget), and offer a "🔄 Refresh now" action that bypasses the hourly TTL but keeps the cross-window lock and a short cooldown. See [docs/features/GITHUB-ACTIVITY-CACHE.md](../docs/features/GITHUB-ACTIVITY-CACHE.md)
