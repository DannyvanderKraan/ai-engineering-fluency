...
### Features
- The Usage Analysis view's nine tabs now sit under four group tabs — **📊 Usage** (My Activity, Recent Sessions), **📁 Workspace** (Tools & Integrations, Workspace Health, Worktrees), **🐙 GitHub** (Repository PRs, Cloud Agent) and **🎓 Coaching** (Insights, Corrections) — matching the group-tab/leaf-tab strip the Diagnostics view already uses. The flat nine-tab strip wrapped onto a second row at ordinary panel widths and gave no hint that, say, Repository PRs and Cloud Agent are the same kind of thing. Re-opening a group returns you to the tab you last had open in it. Tab ids are unchanged, so saved tab selections, "Take me there" links from What's New, and the worktree notification's "Show Me" action all keep working
- The Usage Analysis "My Activity" tab is now grouped into three labelled bands instead of nine sibling sections in one flat stack: **📊 Overview** (sessions, interaction modes), **💵 Spend & models** (billing coverage, model cost, multi-model, model leaderboard, thinking effort) and **🧠 Context** (context references, context window & long-context pricing). "Thinking Effort (Reasoning)" and "Context Window & Long-Context Pricing" previously trailed off the bottom of the tab with nothing to say they answered a different question from the cost sections above them. The full view → sub-view → content-block map, and the rules for when to use a band, leaf tabs or a new view, are in [docs/vscode-extension/VIEW-HIERARCHY.md](../docs/vscode-extension/VIEW-HIERARCHY.md)
- The Usage Analysis "Context References" table now lists only the reference kinds you have actually used — anything with no usage today *and* none in the last 30 days collapses into an "Other references" disclosure, the same way the model leaderboard's long tail already did. The table enumerates all 21 kinds the extension can detect, so on most machines the majority of rows were permanently zero. Nothing is dropped: the group expands, and the totals row still sums every kind
- The automatic-compactions card in the Usage Analysis "Context Window & Long-Context Pricing" section no longer sits between the per-period columns and the long-context pricing bar; the bar now follows the columns it belongs to, and the 7-day compaction count follows both under its own "Context compaction" heading
