...
### Features
- Efficiency > Combined now filters by **model vendor**, **model** and **editor** — three dropdowns that default to All and compose as an intersection, so you can see the efficiency/output picture for one model family, one model, or one editor. The vendor is the company that *made* the model (classified from the canonical model id), not the billing source: Copilot-hosted calls still bill as GitHub Copilot and custom endpoints keep their user-entered provider label. Models we cannot place stay visible under "Unclassified". Filtering is instant and entirely client-side, and per-model values are disclosed as attributed rather than observed — see [docs/features/EFFICIENCY-COMBINED-FILTERS.md](../docs/features/EFFICIENCY-COMBINED-FILTERS.md)
