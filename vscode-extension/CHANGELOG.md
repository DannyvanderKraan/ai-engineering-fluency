...
### Features
- The Efficiency view's weekly charts can now be zoomed to **12, 26 or 52 weeks**, and any week can be selected to see what is behind it. The horizon selector sits above the charts on Trends, Combined, Tools & Skills and the Models drift chart; the week is picked either from a labelled selector (keyboard and screen-reader accessible) or by clicking a point on a chart, and a live detail region reports that week's date range, raw sessions/tokens/turns/lines/cost, every derived ratio with its change against the previous week, whether the week is still partial, and what its sample actually supports. Unavailable metrics stay unavailable — they are never shown as zero — and on Models the existing sample-floor, task-mix and mixed-model-session caveats travel with the selected week. Month vs Month cards that have a weekly equivalent can jump to it, and a Cost Attribution model-shift row can open that model on the Models tab; neither filters anything, because volume, efficiency and mix only add up when they are measured over the same population
