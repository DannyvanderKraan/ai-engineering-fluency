...
### Changed
- **Environmental Impact estimates are now based on published research, and for most people they are much lower.** CO₂ and water used to be flat guesses with no source: 0.2 g CO₂e and 0.3 L of water per 1,000 tokens, whatever the model or token type. They are now derived from Jegham et al., *How Hungry is AI?* ([arXiv:2505.09598](https://arxiv.org/abs/2505.09598)), using the approach published by [neuland/tokendashboard-backend](https://github.com/neuland/tokendashboard-backend/blob/main/docs/co2-methodology.md):
  - **Token type matters:** output tokens count fully, fresh input counts 1/20 as much, and cache reads almost nothing.
  - **Model size matters:** each model is scaled from Claude Sonnet by its output-token price (Haiku ⅓×, Opus 1⅔×).
  - **Water** uses the paper's own formula for data-center cooling plus electricity generation.

  **What to expect:** figures drop sharply for cache-heavy agent use (Claude Code, Copilot CLI) — on one real 30-day history CO₂ fell about 38× and water about 3,000×. Sessions without a per-model token breakdown change much less. **Some usage can come out higher than before:** output-heavy work on large models now counts up to 1,400 g CO₂e per million output tokens for Opus (2,800 g for Fable), against the old flat 200 g per million tokens of any kind. Every period, including past ones, is recalculated from your existing data, so trends shift as a whole rather than jumping at the upgrade. The *Calculation & Estimates* section of the Environmental Impact view shows the full derivation and links both sources; see [docs/TRACKABLE-DATA.md](../docs/TRACKABLE-DATA.md)

### Features
- New **AES Workflow Assessment** section in the Fluency Score view: renders GitHub's Agentic Engineering System (AES) self-assessment of a named delivery workflow — a third, separate lens alongside the personal fluency cards and the per-repository Dark Factory Readiness section. A **Decision and next experiment** block sits directly under the posture, before the stock detail, stating what to delegate now, what is deferred, the top actions, and the evidence that would justify reconsidering. Every stock rating also shows whether it was actually verified or only assumed — an unverified rating downgrades the displayed posture to e.g. "Possible stretched agent-native — confirmation needed" rather than stating it more confidently than the evidence supports. There is no in-product authoring flow yet, so it always shows the fictional "FableCart" example, clearly labelled; a real assessment can be rendered today with the CLI's `aes --file <path>` command. See [docs/features/AES-WORKFLOW-ASSESSMENT.md](../docs/features/AES-WORKFLOW-ASSESSMENT.md)
...
### Bug Fixes
- **AES posture is now only driven by the two foundation stocks (governance, shared knowledge), not customer value.** Previously, a weak or unknown customer-value rating alone could flip the reported posture (e.g. to "stretched agent-native" or "unclear") even when governance and shared knowledge were solid. The FableCart fixture now demonstrates the "stretched agent-native" posture through a genuinely weak, unverified governance rating rather than through customer value
- The AES section's rating/state/confidence badges now derive their CSS class from a fixed allowlist instead of interpolating the assessment's own values directly, closing a possible attribute-injection path for a maliciously crafted assessment file; activity notes are now rendered alongside description and signal (previously dropped)
- The AES section's own headings, notices and footer text are now localized instead of hardcoded English literals
