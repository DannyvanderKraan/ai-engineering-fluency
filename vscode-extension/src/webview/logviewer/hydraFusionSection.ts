...
<th scope="col">${escapeHtml(localize('hydrafusion.table.cost'))}</th>
...
<span title="${escapeHtml(localize('hydrafusion.turn.costTooltip'))}"><strong>${escapeHtml(formatFusionCost(turn.aiu))}</strong></span>
...
${canJumpToStep ? `<span class="hydra-jump-to-step" data-turn="${chatTurnNumber}" title="${escapeHtml(localizeFormat('hydrafusion.turn.jumpToStepTooltip', chatTurnNumber!))}" role="button" tabindex="0">${escapeHtml(localizeFormat('hydrafusion.turn.jumpToStepLabel', chatTurnNumber!))}</span>` : ''}
</span>
</summary>
...
<div class="hydra-panel-sub">${escapeHtml(localize('hydrafusion.turnsPanel.subtitle'))}</div>
