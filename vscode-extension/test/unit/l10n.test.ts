...
test('l10n: Efficiency horizon and week drill-down labels resolve in zh-cn', () => {
	...
	try {
		const expected: Record<string, string> = {
			...
			'efficiency.week.steady': '持平',
			'efficiency.week.priorUnused': '{0} 在 {1} 未被使用，因此没有可供比较的上一周。',
			'efficiency.week.bothModelsHeading': '比较这些模型',
...
test('l10n: every Efficiency drill-down key has a zh-cn translation distinct from the English text', () => {
	...
	const efficiencyKeys = Object.keys(english).filter(k => k.startsWith('efficiency.'));
	assert.ok(efficiencyKeys.length >= 30, `expected the drill-down keys to be present, found ${efficiencyKeys.length}`);
	for (const key of efficiencyKeys) {
